...
def conv3x3(in_planes, out_planes, stride=1):#3x3卷积
    """3x3 convolution with padding"""
    return nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=1, bias=False)

def conv1x1(in_planes, out_planes, stride=1):#1x1卷积
    """1x1 convolution"""
    return nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=stride, bias=False)


class BasicBlock(nn.Module):#残差神经网络基本块
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(BasicBlock, self).__init__()
        self.conv1 = conv3x3(inplanes, planes, stride)# 第一个3x3卷积
        self.bn1 = nn.BatchNorm2d(planes) # 第一个BN层,线性变换,标准化数据分布,学习缩放因子γ和偏移β
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            identity = self.downsample(x)#维度调整
...
class AIDE_Model(nn.Module):
    ...
    def __init__(self, resnet_path, convnext_path):
        ...
        self.hpf = HPF()# 加载30种SRM滤波器
        self.model_min = ResNet(Bottleneck, [3, 4, 6, 3])# ResNet-50结构
        self.model_max = ResNet(Bottleneck, [3, 4, 6, 3])# 另一实例
        ...
        self.fc = Mlp(2048 + 256 , 1024, 2)# 输入: ResNet特征 + ConvNeXt特征的多层感知机

        #print("build model with convnext_xxl")
        self.openclip_convnext_xxl, _, _ = open_clip.create_model_and_transforms(# 加载ConvNeXt-XXL
    ...
    def forward(self, x):
        ...
        x_minmin = x[:, 0] #[b, c, h, w]# 最低频DCT特征
        x_maxmax = x[:, 1] # 最高频DCT特征
        x_minmin1 = x[:, 2]# 次低频DCT特征
        x_maxmax1 = x[:, 3]# 次高频DCT特征
        tokens = x[:, 4]# 原始图像标准化后的版本

        x_minmin = self.hpf(x_minmin)# 30通道高频特征
        ...
        x = torch.cat([x_0, x_1], dim=1)# 拼接语义和频域特征
